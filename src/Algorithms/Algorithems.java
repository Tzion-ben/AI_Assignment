...
public class Algorithems {
	...
	public State_Node  BFS() {
		...
		LinkedTransferQueue<State_Node> howNext = new LinkedTransferQueue<State_Node>();
		...
		while(!openList.isEmpty()) {
			...
			openList.remove(n.getOperation(), n);
...
//			 * */
//			if(result.getNumDirection()!=-1) {return result;}//like: if(result != cutOff)
//		}
//
//		return null;
//	}
...
//		State_Node cutOff =  new State_Node(null, String.valueOf(-1)+","+Direction.N,-1,-1, null, -1, -1);
...
//						if(result.getNumDirection() == 0) {cutOff.setNumDirection(1);}  // cutOff->true
//
//						//if it's not fail and not a cutOff that we found the result
//						else if(result.getNumDirection() != -1 ) {return result;}	
//					}
//					/**
...
//				cutOff.setNumDirection(-1);//-1 mean's fail
//				return cutOff;
//			}
//		}
//	}
	...
	public State_Node A_Star() {
		...
		while(!openList.isEmpty()) {
			...
			HashMap<Integer, Oprerator> allowedOprerators= n.getOprerators();
			for(int i=0 ; i<4 ; i++) {
				Oprerator tempOp = allowedOprerators.get(i);
				Matrix_Variable [][] tempNmatrix = n.deepCopy();
				State_Node child = new State_Node(tempNmatrix, tempOp, n.getG(),n.getH(), n,n.getMinouOne_i(),n.getMinouOne_j());
